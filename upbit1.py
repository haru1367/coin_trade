...
def get_top_gainers_upbit():
    ...
    try:
        ...
        for ticker in tickers:
            ...
            if prev_close > 0:  # 유효한 가격만 계산
                change_percent = ((current_price - prev_close) / prev_close) * 100
                
                # 필터: 증가율이 3% 이상인 항목만 추가
                if change_percent > 3:
                    gainers.append({
                        'symbol': symbol,
...
def get_orderbook(api_key, market="KRW-BTC"):
    ...
    try:
        ...
        # 매도 1호가 (가장 낮은 매수 호가)
        ask_price = orderbook[0]['orderbook_units'][1]['ask_price']
...
while True:
    ...
    if len(coin_holdings)==1:
        ...
        for coin in coin_list:
            ...
            rsi, upper_band, middle_band, lower_band, macd, macd_signal, macd_hist, aroon_up, aroon_down, trix, slow_k, slow_d, adx, cci, stoch_rsi,mfi,aroon_osc,smi,rmi = calculate_indicators(data)
            ...
            while lower_band>status_coin_price and stoch_rsi < 15:
        ...
        rsi, upper_band, middle_band, lower_band, macd, macd_signal, macd_hist, aroon_up, aroon_down, trix, slow_k, slow_d, adx, cci,stoch_rsi,mfi,aroon_osc,smi,rmi = calculate_indicators(get_coin_data(hold_coin))
    
        print(f'{hold_coin}의 현재 가격 : {status_coin_price}원')
        print('')
        print(f'현재 middle band : {middle_band:.8f}')
        print(f'현재 stoch rsi : {stoch_rsi:.2f}')
        ...
        while status_coin_price >= middle_band:
            orderbook = get_orderbook(api_key, market)
            market = f'KRW-{hold_coin}'
            sell_price = orderbook['ask_price'] - coin_order_unit
            sell_uuid = place_limit_sell_order(api_key,secret_key,market,sell_price,hold_coin_amount)
